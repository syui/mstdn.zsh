...
tbody=`cat $json_timeline|jq -r '.[].reblog.account|{avatar,acct,url}'`
...
for ((i=0;i<=$tn;i++))
do
	acct=`cat $json_timeline|jq -r ".[$i].reblog.account|.acct"`
	if [ $i -eq 0 ] ;then
		echo "[" >! $json_avatar_timeline
	fi
	if [ -n "$acct" ];then
		if echo "$acct_tmp"|grep -v "$acct" > /dev/null 2>&1;then
			avatar=`cat $json_timeline|jq -r ".[$i].reblog.account|.avatar"`
			avatar_static=`cat $json_timeline|jq -r ".[$i].reblog.account|.avatar_static"`
			url=`cat $json_timeline|jq -r ".[$i].reblog.account|.url"|cut -d / -f -3`
			if [ "$url" != "null" ];then
